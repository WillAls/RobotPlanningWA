            ...
            for (int i = 0; word[i] != '\0'; i++) {
                ...
                if (currentChar >= 32 && currentChar <= 126) {  // Printable ASCII characters only
                    ...
                    for (int j = 0; j < charData.num_movements; j++) {
                        ...
                        if (m.pen != penState) {
                            penState = m.pen;
                            printf(penState == 1 ? "S1000\n" : "S0\n");  // Pen up or down
                        }

                        // Output movement G-code
                        printf(penState == 1 ? "G1 X%d Y%d\n" : "G0 X%d Y%d\n", newX, newY);
                    }
...
int main() {
    char fontFile[] = "SingleStrokeFont.txt";  // Font data file
    loadFontData(fontFile);  // Load font data from the file

    int height;
    printf("Enter the desired text height (between 4 and 10mm): ");
    scanf("%d", &height);

    if (height < 4 || height > 10) {  // Validate height input
    ...
    FILE *textFile = fopen(textFileName, "r");  // Open the text file
    if (!textFile) {
    ...
    while (fgets(&text[index], sizeof(text) - index, textFile)) {
        index += strlen(&text[index]);  // Advance the buffer index
        if (index >= sizeof(text) - 1) {  // Prevent buffer overflow
            break;
        }
    }

    fclose(textFile);  // Close the text file
    generateGCode(text, (float)height);  // Generate G-code with the input text and height
